...
(use-package counsel
  ...
  :config
    ...
    ;; add file under point to candidate list for counsel-find-file
    ;; (default nil)
    ...
    ;; Want to use the counsel-mark-ring command with C-u C-<, but want
    ;; the pop-to-mark-command under C-<
    ...
    ;; Want to use the counsel-yank-pop command with C-u M-y, but want the
    ;; yank-pop under M-y
    ...
    ;; if rgrep is installed, use that instead of grep for the command
    ;; underlying counsel-grep-or-swiper
   ...
   :bind (
     ;; rebind to what's specified in the ivy file
     ("C-c C-r" . ivy-resume) 
     ;; counsel-ibuffer sucks, use ivy-switch-buffer with ivy-rich instead
     ...
     ;; counsel tries to override C-< with counsel-mark-ring, reset it so
     ;; that requires \\[universal-argument]
     ("C-<" . if-pop-to-mark-or-counsel-mark-with-universal)
     ;; counsel tries to override M-y with counsel-yank-pop, reset it so
     ;; that requires \\[universal-argument]
