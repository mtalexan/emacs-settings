...
(use-package neotree
  ...
  :config
    ...
    ;Jumps to current node every time neo-tree is opened (like autorefresh, but for when tree is usually hidden)
    ...
    ;Confirm changing root if file not found in current one: yes-or-no-p, y-or-n-p, off-p
